...
HTML 상단의 **검토 대상 PDF**는 이번에 조사한 문서다. 상세 근거의 노드 ID·태그·페이지와 조건 안내도 이 문서에서 가져온다. JSON의 `target_source`에 파일명과 PDF/XML 해시가 들어간다.

하단의 **원장 작성 당시 출처**는 원장을 처음 만들 때 참고한 문서다. JSON의 `master_source`에 별도로 남긴다. 두 파일명이 같아도 해시가 다르면 다른 추출본일 수 있다.
...
- 완료 기록 없이 JSON/HTML만 있다고 정상 완료로 취급하지 않는다. `read_completed_item_review`는 완료 기록과 파일 해시를 함께 검증한다.
- HTML의 원문·메모는 일반 텍스트로 표시하며 외부 스크립트나 원격 자료를 불러오지 않는다.
...
- `item_review.html`: 담당자가 브라우저로 열어 읽는 결과. 별도 웹 서버는 필요 없다.
- `item_observation.json`: 현재 근거와 원장 정의가 함께 저장된 기계 판독용 결과.
- `item_review_complete.json`: 위 두 파일이 완성됐고 내용이 일치함을 확인하는 기록. 업무 승인서는 아니다.
...
이번에 추가한 것은 CLI 실행과 JSON/HTML 관찰 결과다. 아직 배포용 Excel writer, Streamlit 연결, 다른 체크리스트의 항목 분리, 모델 조건에 따른 업무 판정은 추가하지 않았다. 작성용 원장 Excel과 검토 결과 Excel은 서로 다른 용도이므로 구분해서 진행한다.
