...
2026-09-13: 항목 검토 결과와 Excel을 **같은 실행 폴더**에 저장한다. 새 실행은 HTML을 만들지 않으며 화면은 폴더 하나에서 JSON과 Excel을 확인한다. Excel Summary는 대상 PDF 파일명까지 12행만 표시하고, 임시 근거 번호 대신 고정 항목 키로 상세 근거를 연결한다. 내부 해시·출처 기록은 JSON에 보존한다. 현재는 구성품 14개 파일럿 결과이며 전체 체크리스트 통합 Excel은 후속 범위다.
