...
**Architecture:** Add a focused Markdown renderer that reads the already-serialized Semantic XML plus `QualityReport.heading_hierarchy`. The output bundle writes the Markdown file into the same staging directory and publishes all four artifacts through a writer-lock-protected, crash-aware transaction with rollback for caught failures. Existing-directory publication replaces files sequentially; concurrent reading during extraction/publication is unsupported, and readers must wait until the extraction command fully exits. Lock observation is not a consistent-snapshot protocol, and hard termination can require manual review.
