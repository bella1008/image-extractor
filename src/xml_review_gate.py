...
CONTRACT = "tagged-pdf-xml/8405120"
EXTRACTOR_SHA256 = "fc64c8bc8e7909b27ed413fc802736b3a48fa5463f6316dba9e43f882ed3aa1e"
...
def validate_bundle(folder: Path, receipt: dict, *, pdf_path: Path, mapping_path: Path) -> CheckedBundle:
    ...
    try:
        require(receipt.get("contract") == CONTRACT, "missing or unsupported extraction contract")
        require(receipt.get("extractor_sha256") == EXTRACTOR_SHA256, "missing or unsupported extractor version hash")
        ...
        require(all(isinstance(d, dict) and d.get("severity") == "warning" for d in diagnostics), "extraction error diagnostic")
        _validate_audit(semantic, report["metrics"]["multilingual_heading_audit"], context)
        return CheckedBundle(context, raw, semantic, report)
...
def _validate_audit(root: ET.Element, audit: dict, context: DocumentContext) -> None:
    ...
    require(audit.get("status") == ("passed" if multi else "not_applicable"), "invalid audit status")
    ...
    require(audit.get("diagnostics") == [] and audit.get("mismatch_positions") == [], "audit has unresolved findings")
    ...
    for key in flags:
        require(audit.get(key) is (True if multi else None), f"failed audit component: {key}")
    ...
    require(len(node) == len(languages), "XML/report audit child mismatch")
    previous = None
    for ordinal, (xml_language, item) in enumerate(zip(node, languages), 1):
