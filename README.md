...
2026-09-20 최신 추출기 `8134892` 통합·회귀 검증은 [통합 기록](docs/migration/2026-09-20-xml-integration_kr.md)을 따른다.
새 실행은 최신 계약을 사용하며 과거 결과 읽기 호환은 유지한다. 체크리스트 운영 범위는 여전히 ZC ENG 파일럿이다.
...
14개 구성품별 JSON을 만들고, 원장 작성 당시 출처와 현재 문서의 근거를 분리한다.
...
2026-09-14: 통합 화면에 **로컬 PDF 경로 → 체크리스트 실행 → 새 폴더에 Excel/JSON → 자동 결과 표시**를 연결했다.
현재 ZC ENG 체크리스트 관찰 파일럿이며, 네 agent 전체의 결과가 아니다.
[결과 파일 의미·화면 실행 방법](docs/migration/2026-09-14-checklist-ui-run_kr.md)을 참고한다.
