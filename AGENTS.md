...
- User requirement (2026-09-13): whenever source-PDF completeness or structure needs human verification, explicitly request it with buyer/source_token, language, exact PDF/extraction version, comparison artifacts and pages/items to check. Do not treat silence, continuation permission, automatic gates or legacy GridCell approval as XML source-content approval. Pending human checks block dependent operational acceptance/DB approval, not independent development.
- Maintain `docs/migration/buyer-language-validation-ledger_kr.md` for every profile/language and exact PDF/run reviewed. Keep extraction checks, adapter preservation checks, human partial/full verification, request status, reviewer/date/evidence and unresolved/recheck scope separate. Register unexamined profiles/languages and unassigned cover regions; reconcile new mapping entries and other-worktree evidence before claiming coverage. Changed source/extraction invalidates affected approval unless unchanged scope is demonstrated and linked.
