...
@pytest.mark.parametrize('buyer,folder,filename,languages,headings',[
 ('MENA_L02','2_TV_MENA','BN68-25031L-00_SUG_Y26 TV ALL_MENA_L02_260114.0.pdf',['ENG','ARA'],8),
 ('XL_ENG','2_TV_XL','BN68-25031J-00_SUG_Y26 TV ALL_XL_ENG_260306.0.pdf',['ENG'],4),
 ('XT_L02','2_TV_XT','BN68-25031M-00_SUG_Y26 TV ALL_XT_L02_260113.0.pdf',['ENG','THA'],8)])
def test_actual_xml_dispatch_complete_buyer_source(tmp_path,buyer,folder,filename,languages,headings):
    source=next((p/'samples/SUG_RAW'/folder/filename for p in POC.parents if (p/'samples/SUG_RAW'/folder/filename).is_file()),None)
    if source is None:pytest.skip('Reviewed source sample unavailable')
